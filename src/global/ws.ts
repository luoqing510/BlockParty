...
export class WS {
    ...
    static init(_this: WS) {
        if (_this.isWebsocket()) {
            _this.websocket!.onopen = (e: any) => {
                _this.openFn(e);
            };

            _this.websocket!.onerror = (e: any) => {
                _this.errorFn(e);
            };
            _this.websocket!.onmessage = (e: any) => {
                _this.messageFn(e);
            };
        } else {
    ...
    onOpen(callBack: (arg0: Event) => void) {
        if (this.isWebsocket()) {
            //判断是否传递回调函数
    ...
    onMessage(callBack: (arg0: MessageEvent<any>) => void) {
        if (this.isWebsocket()) {
            if (typeof callBack == "function") {
    ...
    // 自定义WSC异常事件：WSC报错后触发
    onError(callBack: (arg0: Event) => void) {
        if (this.isWebsocket()) {
            if (typeof callBack == "function") {
    ...
    //前端向服务端发送消息时触发
    onSend(data: string | ArrayBufferLike | Blob | ArrayBufferView) {
